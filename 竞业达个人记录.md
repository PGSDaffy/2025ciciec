...
2. 阅读nemu：
运行nemu之后输入的`c`或`si`都是在调用`cpu_exec(n)`去执行n条指令，而`cpu_exec`则通过调用`execute`来执行，除去其中的`trace`和`difftest`相关代码，执行部分调用了`exec_once`。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTY3NjAyNzIwNywxMTI0NTg1MDgyLDEyMz
