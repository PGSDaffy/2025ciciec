...
3. 添加指令：
了解清楚逻辑之后，读`INSTPAT`中自带的一些指令：`auipc``lbu``sb``ebreak``inv`搞明白了应该如何添加，就是在`operand`中加字符处理的函数，并在`INSTPAT`里面添加对应的之一
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwMTUwNjg3NzAsMTE5MDkyNDc2NiwtNj
