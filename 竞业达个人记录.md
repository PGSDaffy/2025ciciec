...
2. 学了一下桶型移位器，复习了一下加法器的写法，感觉确实应该如讲义所说，尽量别用行为级建模，至少像这种ALU和控制的模块不行，否则的话没办法去做这些微架构新房功能肯定爆炸。明天看先写个大概的架构图：
```mermaid
flowchart TB
    %% 定义模块
    PC[程序计数器\nPC]
    ...
    RF --> |rs1数据| ALU
    RF --> |rs2数据| MUX_ALUSrc
    ImmGen --> |扩展后的立即数| MUX_ALUSrc

    MUX_ALUSrc --> |操作数2| ALU
    CU --> |ALUSrc| MUX_ALUSrc

    ALU --> |结果| DM
    ALU --> |结果| MUX_MemtoReg
    ALU --> |Zero标志| MUX_PCSrc

    DM --> |读取数据| MUX_MemtoReg
    CU --> |MemRead/MemWrite| DM

    MUX_MemtoReg --> |写回数据| RF
    CU --> |MemtoReg| MUX_MemtoReg
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTgzMDM0OTQ5NCwtOTk1MDg1Nzk3LDMxNj
I2Njg5OSwtMzUwNzI5NDg2LC04OTUxOTkzNzMsLTIyMzEwNjA1
LC0yMDYwNDA1ODk1LC0xNDcxOTY5MTksMTM4NTA5NTA4MSwxNz
E4NjQyNjI5LDc2OTY1ODA5OSwxNjUzMjc5Njc4LC0xNjM2MTAy
OTc2LC0xNDg4MDkyMTc5LDExNDkxMTgzMzYsMTE5MDkyNDc2Ni
wtNjc2MDI3MjA3LDExMjQ1ODUwODIsMTIzODM4NDYyOSwtNDUx
NTIyNTAxXX0=
-->